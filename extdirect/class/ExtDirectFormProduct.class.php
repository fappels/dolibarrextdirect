	...
	public function readWarehouses(stdClass $params)
	{
		...
		if (isset($params->filter)) {
			foreach ($params->filter as $key => $filter){
				if ($filter->property == 'productid') $fk_product=$filter->value;
			}
		}
		if (($result = $this->loadWarehouses($fk_product)) < 0)	return $result;
		...
		if (!$this->error) {
			foreach ($this->cache_warehouses as $warehouseId => $warehouse) {
				$row=null;
				$row->id = $warehouseId;
				$row->label= $warehouse['label'];
				$row->stock_reel= $warehouse['stock'];
